/**
 * 集成版GraphRAG Web应用JavaScript模块
 * 结合基础版问答功能和增强版知识点提取功能
 */
...
let queryHistory = [];
let enhancedKnowledgeExtraction = null;
...
function shareAnswer() {
    ...
    const questionElement = answerContent.querySelector('.alert-info strong');
...
function displayEnhancedGraph(graphData) {
    ...
    if (!neo4jGraph) {
        ...
        // 初始化性能优化器
        if (typeof GraphPerformanceOptimizer !== 'undefined') {
            performanceOptimizer = new GraphPerformanceOptimizer(neo4jGraph);
        }
        
        // 初始化交互增强器
        if (typeof GraphInteractionEnhancer !== 'undefined') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 初始化增强版知识点提取模块
    if (typeof EnhancedKnowledgeExtraction !== 'undefined') {
        enhancedKnowledgeExtraction = new EnhancedKnowledgeExtraction();
    }
});
...
function switchToQuery() {
    ...
    const queryBtn = document.getElementById('query-switch-btn');
    const extractionBtn = document.getElementById('extraction-switch-btn');
...
// 导出切换函数
window.switchToQuery = switchToQuery;
window.switchToExtraction = switchToExtraction;